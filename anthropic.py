...
async def create_request_payload(
    ...
    # Refresh admin defaults snapshot so UserValves.model_validator picks up the
    # latest admin configuration when this helper is invoked outside of pipe().
    Pipe._current_admin_defaults = {
        field: getattr(pipe.valves, field)
        for field in _ADMIN_USERVALVE_DEFAULTS.values()
        if hasattr(pipe.valves, field)
    }
    ...
    if (
        activate_code_execution
        and not has_code_execution
        and not has_native_terminal_tools
    ):
    ...
    if any(isinstance(_t, dict) and _t.get("defer_loading") for _t in tools_list):
        ...
        if isinstance(system_messages, list):
            ...
            system_messages = [
                {"type": "text", "text": str(system_messages)},
                _tool_search_nudge,
            ]
        else:
...
# Registry mapping a UserValves field name -> the corresponding admin Valves field name
# whose value should be used as the UserValves default when the user has not explicitly
# set their own value. Updated whenever a Pipe instance is (re)initialized.
_ADMIN_USERVALVE_DEFAULTS: Dict[str, str] = {
    "ENABLE_THINKING": "DEFAULT_ENABLE_THINKING",
    "THINKING_BUDGET_TOKENS": "DEFAULT_THINKING_BUDGET_TOKENS",
    "THINKING_DISPLAY": "DEFAULT_THINKING_DISPLAY",
    "EFFORT": "DEFAULT_EFFORT",
    "USE_PDF_NATIVE_UPLOAD": "DEFAULT_USE_PDF_NATIVE_UPLOAD",
    "SHOW_TOKEN_COUNT": "DEFAULT_SHOW_TOKEN_COUNT",
    "WEB_SEARCH_MAX_USES": "DEFAULT_WEB_SEARCH_MAX_USES",
    "WEB_FETCH_MAX_USES": "DEFAULT_WEB_FETCH_MAX_USES",
    "ENABLE_DYNAMIC_FILTERING": "DEFAULT_ENABLE_DYNAMIC_FILTERING",
    "ENABLE_TOOL_SEARCH": "DEFAULT_ENABLE_TOOL_SEARCH",
    "TOOL_SEARCH_TYPE": "DEFAULT_TOOL_SEARCH_TYPE",
    "TOOL_SEARCH_MAX_DESCRIPTION_LENGTH": "DEFAULT_TOOL_SEARCH_MAX_DESCRIPTION_LENGTH",
    "ENABLE_ADVISOR_TOOL": "DEFAULT_ENABLE_ADVISOR_TOOL",
    "ADVISOR_MODEL": "DEFAULT_ADVISOR_MODEL",
    "ADVISOR_MAX_USES": "DEFAULT_ADVISOR_MAX_USES",
    "ADVISOR_CACHING": "DEFAULT_ADVISOR_CACHING",
    "USE_FILES_API": "DEFAULT_USE_FILES_API",
    ...
    "CONTEXT_EDITING_STRATEGY": "DEFAULT_CONTEXT_EDITING_STRATEGY",
    "CONTEXT_EDITING_THINKING_KEEP": "DEFAULT_CONTEXT_EDITING_THINKING_KEEP",
    "CONTEXT_EDITING_TOOL_TRIGGER": "DEFAULT_CONTEXT_EDITING_TOOL_TRIGGER",
    "CONTEXT_EDITING_TOOL_KEEP": "DEFAULT_CONTEXT_EDITING_TOOL_KEEP",
    "CONTEXT_EDITING_TOOL_CLEAR_AT_LEAST": "DEFAULT_CONTEXT_EDITING_TOOL_CLEAR_AT_LEAST",
    "CONTEXT_EDITING_TOOL_CLEAR_TOOL_INPUT": "DEFAULT_CONTEXT_EDITING_TOOL_CLEAR_TOOL_INPUT",
    "TOOL_RESULT_MAX_TOKENS": "DEFAULT_TOOL_RESULT_MAX_TOKENS",
    "TOOL_SEARCH_EXCLUDE_TOOLS": "DEFAULT_TOOL_SEARCH_EXCLUDE_TOOLS",
}
...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        # --- UserValves defaults (admins may override these as deployment defaults) ---
        DEFAULT_ENABLE_THINKING: bool = Field(default=False, description="Default for UserValves.ENABLE_THINKING when the user has not set it.")
        DEFAULT_THINKING_BUDGET_TOKENS: int = Field(default=8192, ge=1024, le=64000, description="Default for UserValves.THINKING_BUDGET_TOKENS when the user has not set it.")
        DEFAULT_THINKING_DISPLAY: Literal["summarized", "omitted"] = Field(default="omitted", description="Default for UserValves.THINKING_DISPLAY when the user has not set it.")
        DEFAULT_EFFORT: Literal["low", "medium", "high", "xhigh", "max"] = Field(default="high", description="Default for UserValves.EFFORT when the user has not set it.")
        DEFAULT_USE_PDF_NATIVE_UPLOAD: bool = Field(default=True, description="Default for UserValves.USE_PDF_NATIVE_UPLOAD when the user has not set it.")
        DEFAULT_SHOW_TOKEN_COUNT: Literal["Off", "On", "With Cache"] = Field(default="Off", description="Default for UserValves.SHOW_TOKEN_COUNT when the user has not set it.")
        DEFAULT_WEB_SEARCH_MAX_USES: int = Field(default=5, ge=1, le=20, description="Default for UserValves.WEB_SEARCH_MAX_USES when the user has not set it.")
        DEFAULT_WEB_FETCH_MAX_USES: int = Field(default=5, ge=1, le=20, description="Default for UserValves.WEB_FETCH_MAX_USES when the user has not set it.")
        DEFAULT_ENABLE_DYNAMIC_FILTERING: bool = Field(default=False, description="Default for UserValves.ENABLE_DYNAMIC_FILTERING when the user has not set it.")
        DEFAULT_ENABLE_TOOL_SEARCH: bool = Field(default=True, description="Default for UserValves.ENABLE_TOOL_SEARCH when the user has not set it.")
        DEFAULT_TOOL_SEARCH_TYPE: Literal["regex", "bm25"] = Field(default="bm25", description="Default for UserValves.TOOL_SEARCH_TYPE when the user has not set it.")
        DEFAULT_TOOL_SEARCH_MAX_DESCRIPTION_LENGTH: int = Field(default=100, ge=10, le=10000, description="Default for UserValves.TOOL_SEARCH_MAX_DESCRIPTION_LENGTH when the user has not set it.")
        DEFAULT_ENABLE_ADVISOR_TOOL: bool = Field(default=False, description="Default for UserValves.ENABLE_ADVISOR_TOOL when the user has not set it.")
        DEFAULT_ADVISOR_MODEL: Literal["claude-opus-4-7", "claude-opus-4-8", "claude-fable-5", "claude-mythos-5"] = Field(default="claude-opus-4-8", description="Default for UserValves.ADVISOR_MODEL when the user has not set it.")
        DEFAULT_ADVISOR_MAX_USES: int = Field(default=0, ge=0, le=100, description="Default for UserValves.ADVISOR_MAX_USES when the user has not set it.")
        DEFAULT_ADVISOR_CACHING: Literal["off", "5m", "1h"] = Field(default="off", description="Default for UserValves.ADVISOR_CACHING when the user has not set it.")
        DEFAULT_USE_FILES_API: bool = Field(default=False, description="Default for UserValves.USE_FILES_API when the user has not set it.")
        DEFAULT_SKILLS: List[str] = Field(default_factory=list, description="Default for UserValves.SKILLS when the user has not set it.")
        DEFAULT_ENABLE_COMPACTION: bool = Field(default=False, description="Default for UserValves.ENABLE_COMPACTION when the user has not set it.")
        DEFAULT_COMPACTION_TRIGGER_TOKENS: int = Field(default=50000, ge=50000, le=1000000, description="Default for UserValves.COMPACTION_TRIGGER_TOKENS when the user has not set it.")
        DEFAULT_COMPACTION_INSTRUCTIONS: str = Field(default="", description="Default for UserValves.COMPACTION_INSTRUCTIONS when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_STRATEGY: Literal["none", "clear_tool_results", "clear_thinking", "clear_both"] = Field(default="none", description="Default for UserValves.CONTEXT_EDITING_STRATEGY when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_THINKING_KEEP: int = Field(default=0, ge=0, le=9999, description="Default for UserValves.CONTEXT_EDITING_THINKING_KEEP when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_TOOL_TRIGGER: int = Field(default=50000, ge=1000, le=500000, description="Default for UserValves.CONTEXT_EDITING_TOOL_TRIGGER when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_TOOL_KEEP: int = Field(default=5, ge=0, le=100, description="Default for UserValves.CONTEXT_EDITING_TOOL_KEEP when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_TOOL_CLEAR_AT_LEAST: int = Field(default=10000, ge=0, le=100000, description="Default for UserValves.CONTEXT_EDITING_TOOL_CLEAR_AT_LEAST when the user has not set it.")
        DEFAULT_CONTEXT_EDITING_TOOL_CLEAR_TOOL_INPUT: bool = Field(default=False, description="Default for UserValves.CONTEXT_EDITING_TOOL_CLEAR_TOOL_INPUT when the user has not set it.")
        DEFAULT_TOOL_RESULT_MAX_TOKENS: int = Field(default=50000, ge=0, description="Default for UserValves.TOOL_RESULT_MAX_TOKENS when the user has not set it.")
        DEFAULT_TOOL_SEARCH_EXCLUDE_TOOLS: List[str] = Field(default_factory=list, description="When non-empty, REPLACES the UserValves default excluded-tools list with this admin-supplied list.")
    ...
    class UserValves(BaseModel):
        ...
        @classmethod
        def _apply_admin_defaults(cls, values: Any) -> Any:
            """Populate unset UserValves fields from the admin-configured defaults.

            Reads the latest admin defaults from Pipe._current_admin_defaults
            (refreshed by Pipe.__init__ and pipe on each request). If the
            incoming values already contains a field, it wins; if not, the
            admin default is used. For TOOL_SEARCH_EXCLUDE_TOOLS the admin
            default only overrides when non-empty (admin opting out leaves the
            user the original long default list).
            """
            if not isinstance(values, dict):
                return values
            admin_defaults = getattr(Pipe, "_current_admin_defaults", None) or {}
            for user_field, admin_field in _ADMIN_USERVALVE_DEFAULTS.items():
                if user_field in values and values[user_field] not in (None, ""):
                    continue
                if user_field == "TOOL_SEARCH_EXCLUDE_TOOLS":
                    admin_val = admin_defaults.get(admin_field) or []
                    if not admin_val:
                        continue
                else:
                    if admin_field not in admin_defaults:
                        continue
                    admin_val = admin_defaults[admin_field]
                if admin_field not in admin_defaults:
                    continue
                values[user_field] = admin_val
            return values
    ...
    # Snapshot of admin Valves defaults exposed to UserValves.model_validator.
    # Updated by every Pipe.__init__ and re-set at the start of pipe() so the
    # latest admin configuration drives user defaults even if Pipe is long-lived.
    _current_admin_defaults: Dict[str, Any] = {}
    ...
    def __init__(self):
        ...
        Pipe._current_admin_defaults = {
            field: getattr(self.valves, field)
            for field in _ADMIN_USERVALVE_DEFAULTS.values()
            if hasattr(self.valves, field)
        }
        self.logger = logger
    ...
    def _convert_tools_to_claude_format(
        ...
        bash_active = (
            self.valves.ENABLE_BASH_TOOL and has_run_command and terminal_active
        )
        text_editor_active = (
        ...
        _defer_active = (
            __user__["valves"].ENABLE_TOOL_SEARCH and not is_programmatic_active
        )
        ...
        for claude_tool in claude_tools:
            ...
            if _defer_active:
                ...
                if name != forced_tool_name and name not in user_excludes:
    ...
    def _convert_tool_result_content(self, result_str, user=None):
        ...
        for match in matches:
            ...
            if prefix.strip():
                blocks.append(
                    {
                        "type": "text",
                        "text": self._truncate_tool_result_text(prefix, user),
                    }
                )
            blocks.append(
                self._build_tool_result_image_block(
                    match.group("mime"), match.group("data"), user
                )
            )
            last_end = match.end()
        ...
        if suffix.strip():
            blocks.append(
                {"type": "text", "text": self._truncate_tool_result_text(suffix, user)}
            )
    ...
    def _build_tool_result_image_block(
        self, mime_type: str, encoded: str, user=None
    ) -> dict:
        ...
        if PIL_AVAILABLE:
            try:
                ...
                logger.debug(
                    f" Tool result image resize failed, sending original: {resize_ex}"
                )
                final_bytes = decoded_bytes
                final_media_type = media_type
    ...
    async def _generate_file_download_link(
        ...
        try:
            ...
            _, file_path = Storage.upload_file(
                io.BytesIO(content), storage_filename, {}
            )
    ...
    async def _await_tool_task_result(
        ...
        if timeout_s is None:
            timeout_s = getattr(
                self.valves, "TOOL_CALL_TIMEOUT", self.TOOL_CALL_TIMEOUT
            )
        try:
    ...
    async def get_anthropic_models(self) -> List[dict]:
        ...
        try:
            ...
            if not models:
                logger.warning(
                    "Model listing returned no models; using static fallback"
                )
    ...
    def _build_anthropic_client(
        ...
        headers = {
            k: v for k, v in (default_headers or {}).items() if k.lower() != "x-api-key"
        }
    ...
    async def pipe(
        ...
        # Refresh admin defaults snapshot so the UserValves model_validator
        # picks up any in-place edits to self.valves.
        Pipe._current_admin_defaults = {
            field: getattr(self.valves, field)
            for field in _ADMIN_USERVALVE_DEFAULTS.values()
            if hasattr(self.valves, field)
        ...
        try:
            ...
            client = self._build_anthropic_client(
                api_key, default_headers=headers, timeout=request_timeout
            )
        ...
        # The Open WebUI runtime emits its own completion status ("🅺 Response Complete")
        # when the streamed response ends. Emitting another done=True status here
        # results in a duplicate "Response Complete" notification in the chat footer.
        # Skip the redundant emit; non-streaming callers still get feedback through
